<p align="center">
  <img src="public/favicon.ico" width="65" alt="Logo">
    <a href="https://laravel.com" target="_blank"><img src="https://raw.githubusercontent.com/laravel/art/master/logo-lockup/5%20SVG/2%20CMYK/1%20Full%20Color/laravel-logolockup-cmyk-red.svg" width="240" alt="Laravel Logo"></a>
</p>

<h1 align="center">AmikomEventHub</h1>

<p align="center">
  Platform manajemen event dan pemesanan tiket berbasis web untuk lingkungan kampus Amikom. <br>
  Dibangun menggunakan Laravel dan Tailwind CSS dengan antarmuka admin yang modern dan responsif.
</p>

---

## 🛠️ Tech Stack

| Teknologi                                                                                                                    | Deskripsi                           |
| ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| ![Laravel](https://img.shields.io/badge/laravel-%23FF2D20.svg?style=for-the-badge&logo=laravel&logoColor=white)              | Backend framework utama             |
| ![PHP](https://img.shields.io/badge/php-%23777BB4.svg?style=for-the-badge&logo=php&logoColor=white)                          | Bahasa pemrograman server-side      |
| ![TailwindCSS](https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=for-the-badge&logo=tailwind-css&logoColor=white) | Utility-first CSS framework         |
| ![Postgres](https://img.shields.io/badge/postgres-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white)         | Database relasional                 |
| ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) | Interaktivitas frontend (AJAX, DOM) |

---

## 📋 Tentang Project

**AmikomEventHub** adalah aplikasi web untuk mengelola dan mempromosikan event kampus. Aplikasi ini menyediakan halaman publik untuk menjelajahi event yang tersedia, melakukan checkout tiket, serta panel admin yang lengkap untuk mengelola data event, kategori, dan transaksi.

### ✨ Fitur Utama

- 🏠 Halaman utama dengan daftar event
- 📄 Halaman detail event
- 🛒 Sistem checkout & pemesanan tiket
- 🎛️ Dashboard admin
- 📦 CRUD Event (Create, Read, Update, Delete)
- 🏷️ CRUD Kategori Event
- 💳 Manajemen transaksi

---

## 👤 Identitas

| **Nama** | Alief Fathin Adi Kusuma |
| -------- | ----------------------- |
| **NIM**  | 24.12.3303              |

<a href="https://amikomeventhub-3303.free.laravel.cloud/" target="_blank"><img src="https://img.shields.io/badge/🚀_Live_Demo-Visit_Site-4F46E5?style=for-the-badge" alt="Live Demo"></a>

---

## 🗺️ Daftar URL

### Halaman Publik

| Halaman            | Deskripsi                                | Link                                                |
| ------------------ | ---------------------------------------- | --------------------------------------------------- |
| Home               | Halaman utama — menampilkan daftar event | https://amikomeventhub-3303.free.laravel.cloud/     |
| Kategori           | Halaman menampilkan berdasarkan kategori |                                                     |
| About              | Halaman Tentang Kami                     |                                                     |

### Halaman Admin

| **Halaman**          | **Deskripsi**                              | **Link**                                                        |
| -------------------- | ------------------------------------------ | --------------------------------------------------------------- |
| Dashboard            | Dashboard admin dengan ringkasan statistik | https://amikomeventhub-3303.free.laravel.cloud/admin/dashboard  |
| Kelola Event         | Daftar semua event                         | https://amikomeventhub-3303.free.laravel.cloud/admin/event      |
| Kelola Kategori      | Daftar semua kategori                      | https://amikomeventhub-3303.free.laravel.cloud/admin/categories |
| Transaksi            | Laporan Transaksi                          | https://amikomeventhub-3303.free.laravel.cloud/admin/transaksi  |

---

<p align="center">
  &copy; 2026. Alief Fathin Adi Kusuma - All rights reserved.
</p>
