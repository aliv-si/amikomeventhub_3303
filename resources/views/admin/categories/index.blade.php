    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="categories-table-body" class="divide-y border-t transition-opacity duration-300">
                    @forelse($categories as $index => $category)
                    <tr class="hover:bg-indigo-50/50 transition cursor-pointer group"
                        onclick="window.location='{{ route('admin.event', ['category_id' => $category->id]) }}'">
                        ...
                        <td class="px-8 py-6" onclick="event.stopPropagation()">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.categories.edit', $category->id) }}" class="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" onsubmit="return handleDelete(event)" class="delete-form inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-8 py-10 text-center text-slate-500 font-medium">Belum ada kategori yang ditambahkan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
