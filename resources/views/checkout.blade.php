...
@php
$orderId = 'TRX-' . time() . '-' . \Illuminate\Support\Str::random(5);
@endphp
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                ...
                <div>
                    ...
                    <p class="text-slate-500">{{ \Carbon\Carbon::parse($event->date)->translatedFormat('d F Y') }} • {{ $event->location }}</p>
                    <p class="text-indigo-600 font-bold mt-2">
                        1 x {{ $event->price > 0 ? 'Rp ' . number_format($event->price, 0, ',', '.') : 'Gratis' }}
                    </p>
                </div>
            </div>
            ...
            <form id="checkout-form" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Nama
                        Lengkap</label>
                    <input type="text" id="buyer_name" name="customer_name" placeholder="Masukkan nama sesuai identitas" value="{{ session('google_name') ?? '' }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Email
                            Aktif</label>
                        <input type="email" id="buyer_email" name="customer_email" placeholder="contoh@gmail.com" value="{{ session('google_email') ?? '' }}"
                            class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                            required>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">No.
                            WhatsApp</label>
                        <input type="tel" id="buyer_phone" name="customer_phone" placeholder="08xxxxxxx"
                            class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                            required>
                    </div>
                </div>
                ...
                <button type="button" onclick="showMidtrans()" id="btn-pay"
...
<div id="midtrans-overlay"
    ...
    <div class="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl animate-bounce-in">
        <div class="bg-slate-50 p-6 flex justify-between items-center border-b">
            <img src="{{ asset('img/logo.svg') }}" alt="Midtrans Logo" class="h-5">
        ...
        <div class="p-8 text-center">
            ...
            <p class="text-xs text-slate-400" id="midtrans-order-id">Order ID #TRX-{{ $orderId }}</p>
            ...
            <div class="mt-8 space-y-4">
                ...
                <button onclick="goToTicket()"
                    class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 hover:border-indigo-600 transition group">
                    <span class="font-bold group-hover:text-indigo-600">Virtual Account (BNI, BRI)</span>
                    <span class="text-indigo-400">→</span>
                </button>
                <button onclick="goToTicket()"
                    class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 hover:border-indigo-600 transition group">
...
<script>
    function goToTicket() {
        ...
        const snapToken = window.currentSnapToken;
        ...
        // Sembunyikan dummy overlay terlebih dahulu
        hideMidtrans();

        // Buka popup Midtrans Snap
        snap.pay(snapToken, {
            onSuccess: function(result) {
                window.location.href = '{{ url("/success") }}/' + orderId;
            },
